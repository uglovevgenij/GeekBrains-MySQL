...
CHECK (from_user_id != to_user_id); -- добавляем ограничение, что один юзер не может сам себя добавить

SHOW CREATE TABLE users;

ALTER TABLE users MODIFY COLUMN phone CHAR(11) NOT NULL; -- добавляем условие, в котором телефон будет только из 11 знаков
ALTER TABLE users
ADD CONSTRAINT phone_check
CHECK (LENGTH(phone) > 10 AND phone NOT LIKE '%[^0-9]%'); -- 'LIKE' регулярное выражение(условие совпадения), в котором хотим, чтобы в телефоне были только данные цифры, AND - оба условия, OR - одно из условий


SHOW CREATE TABLE messages;

ALTER TABLE messages MODIFY COLUMN updated_at DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP; -- добавляем условие, чтобы при редактировании обновлять время

SHOW CREATE TABLE profiles;

ALTER TABLE profiles MODIFY COLUMN photo_id BIGINT UNSIGNED DEFAULT NULL UNIQUE; -- крепим уникальное значение к фото ИД


SHOW CREATE TABLE media_types;

ALTER TABLE media_types MODIFY COLUMN name VARCHAR(45) NOT NULL UNIQUE; -- добавляем уникальность названию медиа
